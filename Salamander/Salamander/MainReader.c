...
voids displayBuffer(BufferPointer ptr_Buffer);
num_l getFileSize(word_s fname);
num_i isNumber(const word_s ns);
voids startReader(word_s, word_s, word_c, num_i, num_i);
...
num_i mainReader(num_i argc, word_s* argv) {
	//printf("* mainReader\n"); /* Debug message */
	/* Create source input buffer */
	word_s program = argv[0];
	word_s input = argv[2];
	word_c mode = argv[4];
	num_i size = 0, increment = 0, wrongNumber = 0;
	...
	if (argc == 4) {
		mode = *argv[3];
		switch (mode) {
		case MODE_FIXED: case MODE_ADDIT: case MODE_MULTI: case MODE_TOTAL: break;
		default:
...
************************************************************
*/
voids startReader(word_s program, word_s input, word_c mode, num_i size, num_i increment) {
	//printf("* startReader\n"); /* Debug message */
	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	num_i loadSize = 0;		/* the size of the file loaded in the buffer */
	word_c symbol;			/* symbol read from input file */
