...
num_i errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern num_i line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(voids);
...
voids displayScanner(BufferPointer ptrBuffer);
...
num_i mainScanner(num_i argc, word_s* argv) {
	...
	} while (currentToken.code != SOURCE_END_OF_FILE);
